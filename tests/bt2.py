...
def mtf_backtest_compound(
    ...
    for ticker, (df60_raw, df15_raw) in data_dict.items():
        ...
        if 'ATR14' in df60 and atr_k:
            merge_cols.append('ATR14')
        if ema_col and ema_col in df60:
            merge_cols.append(ema_col)
        ...
        for _, row in df15.iterrows():
            price = row['Close']
            ts    = row['datetime']   # ← 누락돼 있던 부분

            # ─────────────── 진입 ───────────────
            long_entry = (
                not in_pos
                and (row['supertrend_up'] or row['supertrend_up_60'])
                and (ema_col is None or price > row.get(f'{ema_col}_60', 0))
            )
            if long_entry:
                qty = int(cash // price)
                if qty == 0:
            ...
            if in_pos:
                exit_flag = (
                    row['supertrend_down'] or row['supertrend_down_60']
                    or (stop   and price <= stop)
                    or (target and price >= target)
                )
                if exit_flag:
                    cash += stock * price
                    trade = trades_log[ticker][-1]
                    trade['exit_time']  = ts
        ...
        if in_pos and stock:
            ...
            trade['pnl']        = (last_price - trade['entry_price']) * trade['qty']
            stock = 0
    ...
    return balances, total_init, total_final, trades_log

# -------------------------------------------------
# 실행 예시
# -------------------------------------------------
cash = 300_000
balances, total_init, total_final, trades_log = mtf_backtest_compound(
    data_dict,
    initial_cash     = cash,
    atr_k            = 2.0,
    ema_col          = None,   # 'EMA200' 등으로 바꿀 수 있음
    profit_pct_param = None    # 고정 TP 끄기
)
...
# -------------------------------------------------
print("\n===== 종목별 계좌 잔고 결과 =====")
print("종목코드 | 최종 잔고(₩) | 손익(₩) | 손익률(%) | 거래횟수")
print("-" * 65)
for ticker, final_cash in balances.items():
    profit_abs  = final_cash - cash
    profit_rate = (final_cash / cash - 1) * 100
    trade_count = len(trades_log[ticker])
    print(f"{ticker:8} | {final_cash:12,.0f} | {profit_abs:8,.0f} | {profit_rate:9.2f} | {trade_count:8d}")
print("-" * 65)
...
# 2) 종목별 거래내역 CSV 저장
# -------------------------------------------------
for ticker, logs in trades_log.items():
    if not logs:
        print(f"[{ticker}] 거래 내역 없음 → CSV 저장 건너뜀")
        continue

    df_trades = pd.DataFrame(logs)
    filename  = f"{ticker}_trades_.csv"
